...
func TestStreamIgnoresPrefilledBuffer(t *testing.T) {
	...
	clean := render(NewBinauralStream(sr, testChanges(), 1<<30, seededNoise()), n, 512, 0)
	dirty := render(NewBinauralStream(sr, testChanges(), 1<<30, seededNoise()), n, 512, 0.75)
...
func TestStreamThroughMixerIgnoresPriorStreamers(t *testing.T) {
	...
	alone := render(NewBinauralStream(sr, testChanges(), 1<<30, seededNoise()), n, 512, 0)
	...
	m := &beep.Mixer{}
	m.Add(loud, NewBinauralStream(sr, testChanges(), 1<<30, seededNoise()))
...
func TestTonesAudibleWithZeroNoiseVolume(t *testing.T) {
	...
	out := render(NewBinauralStream(sr, changes, 1<<30, seededNoise()), 44100, 512, 0.5)
...
func TestIndependentVolumesAndChannels(t *testing.T) {
	...
	out := render(NewBinauralStream(sr, noiseOnly, 1<<30, seededNoise()), 4410, 512, 0.3)
	var energy float64
	for i, f := range out {
		if f[0] != f[1] {
	...
	out = render(NewBinauralStream(sr, tones, 1<<30, seededNoise()), 44100, 512, 0)
...
func TestParamsMatchInterpolate(t *testing.T) {
	changes := testChanges()
	bs := NewBinauralStream(44100, changes, 1<<30, seededNoise())
